...
def doublet_summary():
    return {"clusters": [
        {"cluster": "6", "concern": "Moderate", "summary": "One high-depth outlier (ACGAACTGGCTATG-1); raw-count screen does not confirm a doublet."},
        {"cluster": "7", "concern": "Moderate", "summary": "High depth and several mixed-programme flags; no confirmed doublet call."}
    ], "method": "Raw-count depth, detected genes, and transcript-supplied programme co-expression screening."}
...
async function loadDoublets(){const d=await get('/api/doublets');$('doubletMethod').textContent=d.method;$('doubletRows').innerHTML=d.clusters.map(x=>`<div class="rounded bg-slate-50 p-3 text-sm"><b>Cluster ${x.cluster}</b> · concern: ${x.concern}<br>${x.summary}</div>`).join('')}
