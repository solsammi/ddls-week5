...
- **Layout:** a decision strip at the top shows the verdict and confidence for clusters 6 and 7 (click one to jump to its evidence). The header carries the talk title, presenter and data owner, and a **Larger text** button for projectors. The page is split into five tabs that follow the talk: **Data & question** (the app opens here: what data was supplied, the owner's two decisions, their definitions and marker clues, all from the interview transcript), **Explore**, **Clusters 6 & 7** (the merge / split / dead-or-empty evidence plus a side-by-side comparison of any two clusters), **Methods & validation** (why each analysis was done and what it showed, the independent reclustering, stability, the doublet screen with its reasoning, and annotations), and **Answers, limits & AI use** (the two answers with confidence, the limits, where each piece of knowledge came from, and how the work was split between Pi, Claude and me). Together these contain everything in the seminar slides, so the app can be presented on its own. On wide screens the map stays in view while you scroll the cluster card.
...
- `GET /api/reclustering` — independent reclustering summary from `results/reclustering_summary.csv` (new in v2).
