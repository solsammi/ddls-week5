...
@app.get("/api/heatmap/{cluster}")
def heatmap(cluster: str, request: Request, n_genes: int = Query(12, ge=1, le=30)):
    ad = dataset(request)
    mask = valid_cluster(ad, cluster)
    genes = list(dict.fromkeys(sum(MARKERS.values(), [])))
    genes = [g for g in genes if g in ad.var_names][:n_genes]
    if not genes:
    ...
    return {"genes": genes, "cells": [str(x) for x in ad.obs_names[mask]], "values": matrix.tolist()}
...
$('colourMode').onchange=()=>$('geneControls').classList.toggle('hidden',$('colourMode').value!=='gene');$('colourBtn').onclick=apply;$('resetBtn').onclick=()=>{plot();$('card').classList.add('hidden')};init().catch(e=>alert(e.message));
</script></body></html>'''
